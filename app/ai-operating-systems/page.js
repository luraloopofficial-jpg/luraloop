    ...
    <div className="luxury-ambient-bg min-h-screen text-zinc-100 selection:bg-orange-600/30 overflow-x-hidden pt-24 pb-16">
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-16 border-t border-zinc-900">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {['Workflow Coordination', 'Trigger-Based Execution', 'System-Aware Routing', 'Department-Level Orchestration', 'Unified Operational Logic', 'Non-Disruptive Integration'].map((item, idx) => (
            <div key={idx} className="bg-zinc-950/40 border border-zinc-900 rounded-xl p-6 hover:border-zinc-800 transition">
              ...
              <p className="text-zinc-400 text-xs leading-relaxed">Enterprise-grade modular framework built for secure cross-system execution.</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-12 border-t border-zinc-900">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {['Zero-Retention Processing', 'Scoped Access Logic', 'Client System Alignment'].map((title, idx) => (
            <div key={idx} className="bg-zinc-950/20 border border-zinc-900/60 p-5 rounded-lg">
              <h4 className="text-white font-semibold text-sm mb-1">{title}</h4>
              <p className="text-zinc-500 text-xs">Scoped permissions aligned directly with enterprise workflow controls.</p>
            </div>
          ))}
        </div>
      </section>
